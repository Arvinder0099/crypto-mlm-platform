...
const AllActiveMembers = () => {
  ...
  // Active members - fetched from API
  ...
  React.useEffect(() => {
    ...
    fetch('/api/admin/members?status=active', {
      ...
      .then(res => res.json())
      .then(data => {
        setActiveMembers(data.members || data.data || []);
        setLoading(false);
      })
      .catch(err => {
  ...
  const handleExport = () => {
    const exportData = activeMembers.map(member => ({
